...
def collate_fn(batch):
    ...
    for text, targets, path in batch:
        ...
        bert_input = tokenizer.batch_encode_plus(text, pad_to_max_length=True, return_tensors='pt', max_length=25)
...
class DRCDdataset(Dataset):
    ...
    def read_DRCD_file(self, path):
        ...
        # targets [(0 or 1)*sentence_num]
        # path [path1, path2, ...]
        
        return text, targets, path
        
# model = maybe_cuda(Model(300,1,1))
# 看transformer的trainer code
class Model(nn.Module):
    def __init__(self, hidden_dim, hidden_layer, batch_size):
        ...
        modules = [self.bert.embeddings, self.bert.encoder.layer[:11]] #Replace 5 by what you want
    ...
    def forward(self, batch):
        ...
        # batched_cls_lhs : [(s1,768),(s2,768),(s3,768)]
        max_doc_len = max(doc_len)
        # 統一padding長度
        # padding提早做
        # 讀文章先固定好句子長度
        # 一次讀一篇句子就好
        padded_doc = [self.pad_document(d,max_doc_len) for d in batched_cls_lhs] # [(Smax, 1,768),(Smax, 1,768),(Smax, 1,768)]
        docs_tensor = torch.cat(padded_doc, 1) # (Smax, 3, 768)
        
        x, _ = self.lstm(docs_tensor) # x : (Smax, 3, hidden_dim)
        
        # 直接給index，loss計算時
        doc_outputs = []
        for i, size in enumerate(doc_len):
            doc_outputs.append(x[0:size, i, :]) # doc_outputs : [(s1, 1, hidden_dim),(s2, 1, hidden_dim),(s3, 1, hidden_dim)]

        x = torch.cat(doc_outputs, 0) # x : (s1+s2+s3, 1, hidden_dim)
        
        x = self.linear(x) # x: (s1+s2+s3, 1, class_num)
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_pk < best_val_pk:
            ...
            logger.debug(
                colored(
                    'Current best model from epoch {} with p_k {} and threshold {}'.format(epoch, val_pk, threshold),
                    'green'))
            best_val_pk = val_pk
            with (checkpoint_path / 'best_model.t7'.format(epoch)).open('wb') as f:
                torch.save(model, f)
